...
export default function LocatarioDashboard() {
  ...
  async function loadContracts() {
    try {
      ...
      const { data, error } = await supabase
        .from('contracts')
        .select('*')
        .eq('locatario_email', userRole.email)
  ...
  const getStatusBadge = (contract: Contract) => {
    ...
    if (contract.locatario_firma_fecha && !contract.locador_firma_fecha) {
      return <span className="px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-800">✓ Firmaste - Esperando locador</span>;
    }
    if (!contract.locatario_firma_fecha && contract.locador_firma_fecha) {
      ...
      <header className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">Vista de Locatario (Inquilino)</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-right">
              <p className="text-sm font-medium text-gray-900">{userEmail}</p>
              <p className="text-xs text-gray-600">Locatario</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className="text-2xl font-bold text-gray-900">
              {contracts.filter(c => !c.locatario_firma_fecha).length}
            </p>
        ...
        {contracts.length === 0 ? (
          <div className="card text-center py-12">
            ...
            <p className="text-gray-600 mb-6">
              Cuando tu inmobiliaria cree un contrato donde figuras como locatario, aparecerá aquí.
            </p>
              ...
              <div key={contract.id} className="card hover:shadow-lg transition">
                <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      {contract.locatario_firma_fecha ? (
                        <span className="text-green-600 flex items-center">
                          ...
                          Firmaste el {formatDate(contract.locatario_firma_fecha)}
                        </span>
                      ) : (
                  ...
                  <div className="ml-4 flex flex-col items-end gap-2">
                    ...
                    {!contract.locatario_firma_fecha && (
                      <Link
                        href={`/firma/${contract.id}?tipo=locatario`}
