...
export default function ProfileClient({ next, initial }: Props) {
  const defaultPlan: PlanChoice = useMemo(() => {
    const p = (initial.planHint || "").toLowerCase();
    if (p.includes("company")) return "company_pro";
    if (p.includes("individual_pro") || p === "pro") return "individual_pro";
    return "free";
  }, [initial.planHint]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!r.ok) {
        setStatus(j?.error || "No se pudo guardar el perfil.");
        return;
      }
    ...
    <form className="space-y-6" onSubmit={onSubmit}>
      {initial.paused ? (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          Tu cuenta está pausada. Completá/actualizá tus datos para continuar.
        </div>
      ) : null}

      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-sm text-zinc-600">Cuenta</div>
          <div className="text-lg font-semibold">{initial.email || "—"}</div>
        </div>
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-zinc-600">
          Elegí el tipo de cuenta para organizar tus documentos. Podés cambiarlo más adelante.
        </p>
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right">
                    <div className="text-sm font-semibold">
                      {p.priceArs === 0
                        ? formatArs(0)
                        : `${p.pricePrefix ? p.pricePrefix + " " : ""}${formatArs(p.priceArs)}`}
                    </div>
                    {p.listPriceArs ? (
